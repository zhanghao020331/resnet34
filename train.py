...
def main():
    ...
    #
    data_transfrom={
        "val":transforms.Compose([transforms.RandomResizedCrop(64),
    ...
    data_root="D:/hua"#图片根目录
    # 判断路径是否存在
    assert os.path.exists(data_root), "{} path does not exist.".format(data_root)
    # 训练集
    train_data=datasets.ImageFolder(root=os.path.join(data_root,"x10"),
    ...
    train_num = len(train_data)
    val_data=datasets.ImageFolder(root=os.path.join(data_root,"x10"),
                                  transform=data_transfrom["train"])
    ...
    print("Using {} for val,using {} for train".format(train_num,val_num))

    net=resnet34()

    weigth_path=r"D:\hua\ResNet-pytorch-main\resnet34.pth"
    ...
    net.fc=torch.nn.Linear(inchannels,20)
    ...
    epochs=3
    best_acc=0.0
    save_path=r"D:\hua\ResNet-pytorch-main\resnet34-1100.pth"
    ...
    for epoch in range(epochs):
        ...
        for step,data in enumerate(train_bar):
            ...
            train_bar.desc="val epoch[{}/{}] loss:{:.3f}".format(epoch+1,epochs,loss)
